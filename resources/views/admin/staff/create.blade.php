...
@section('admin-title', 'کارمند جدید')
@section('admin-page-title', 'کارمند جدید')
@section('admin-page-description', 'در این بخش می‌توانید یک کارمند جدید درست کنید.')

@section('content')

    <form action="{{ route('admin.staffs.store') }}" method="post" name="file" enctype="multipart/form-data" id="upload_form">
        @csrf
        @method('post')
        <div class="card card-flush card-shadow">
            <div class="card-header">
                <h3 class="card-title">کارمند جدید</h3>
            </div>
            <div class="card-body" >
                    ...
                    @foreach( $languages as $lang)
                        <div class="tab-pane fade {{$loop->first?'show active': ''}}" id="tsa_tab_pane_{{$lang->id}}">
                            <x-backend.input.group :lang="$lang" label="نام کارمند" :error="$errors->first('name')">
                                <x-backend.input.text name="name"></x-backend.input.text>
                            </x-backend.input.group>
                            <x-backend.input.group :lang="$lang" label="سمت" :error="$errors->first('position')" >
                                <x-backend.input.text name="position"></x-backend.input.text>
                            </x-backend.input.group>
                            <x-backend.input.group :lang="$lang" label="توضیحات" :error="$errors->first('description')">
                                <x-backend.input.textarea  name="description"  id="service_content"></x-backend.input.textarea>
                            </x-backend.input.group>
                        </div>
                    @endforeach
                </x-backend.tabs>
