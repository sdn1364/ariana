...
@section('admin-title', 'مناقصه جدید')
@section('admin-page-title', 'مناقصه جدید')
@section('admin-page-description', 'در این بخش می‌توانید یک مناقصه جدید درست کنید.')
    ...
    <form action="{{ route('admin.tenders.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="card card-flush card-shadow">
            <div class="card-header">
                <h3 class="card-title">مناقصه جدید</h3>
            </div>
            <div class="card-body">
                    ...
                    @foreach($languages as $lang)
                        <div class="tab-pane fade {{$loop->first?'show active': ''}}" id="tsa_tab_pane_{{$lang->id}}">
                            ...
                            <x-backend.input.group label="خلاصه" :lang="$lang" :error="$errors->first('brief')">
                                <x-backend.input.textarea  type="editor" name="brief" label="خلاصه" id="tender_brief"></x-backend.input.textarea>

                            </x-backend.input.group>
                            <x-backend.input.group :lang="$lang" label="نقش‌ها" :error="$errors->first('roles')">
                                <x-backend.input.textarea name="roles" id="tender_roles"></x-backend.input.textarea>
                            </x-backend.input.group>
                        </div>
                    @endforeach
                </x-backend.tabs>
                <div class="row mt-5">
                    <div class="col">
                        <x-backend.input.group label="کد مناقصه" :error="$errors->first('code')">
                            <x-backend.input.text name="code"></x-backend.input.text>
                        </x-backend.input.group>
                    </div>
                    <div class="col">
                        <x-backend.input.group label="تاریخ ضرب‌الاجل" :error="$errors->first('deadline')">
                            <x-backend.input.datepicker id="tender_deadline" name="deadline"></x-backend.input.datepicker>
                        </x-backend.input.group>
                    </div>
                </div>
                <div class="row">
                    <div class="col">
                        <x-backend.input.group label="وضعیت">
                            ...
                            <x-backend.input.select name="progress">
                                <option value="current">در حال اجرا</option>
                ...
                <x-backend.input.group label="آپلود سند">
                    <x-backend.input.file name="document" id="document"></x-backend.input.file>
                </x-backend.input.group>
                <div class="mb-5">
                    <label class="form-label">تگ‌ها</label>
                    <input class="form-control" value="{{old('tags')}}" id="kt_tagify_1" name="tags"/>
                    @error('tags')
