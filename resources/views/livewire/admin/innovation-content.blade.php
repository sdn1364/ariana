    ...
    <form action="" wire:submit.prevent="save(Object.fromEntries(new FormData($event.target)))">
        <div class="card card-flush">
            <div class="card-header">
                <h3 class="card-title">محتوای صفحه نوآوری</h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-4">
                        ...
                        <x-backend.tabs :languages="$languages">
                            @foreach( $languages as $lang)
                                <div class="tab-pane fade {{$loop->first?'show active': ''}}" id="tsa_tab_pane_{{$lang->id}}">
                                    <x-backend.input.group :lang="$lang" label="عنوان صفحه">
            ...
            @push('scripts')
                ...
                <script>
                    @foreach( $languages as $lang)
                    ClassicEditor
                        .create(document.querySelector('#innovation_content_{{ getLocale($lang) }}'))
                        .then(editor => {
                        })
                        .catch(error => {
                        })
