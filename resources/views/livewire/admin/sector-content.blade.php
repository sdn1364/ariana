    ...
    <form action="" wire:submit.prevent="save(Object.fromEntries(new FormData($event.target)))">
        <div class="card card-flush">
            <div class="card-header">
                <h3 class="card-title">محتوای صفحه اصلی بخش‌ها</h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-4">
                        ...
                        <x-backend.tabs :languages="$languages">
                            @foreach( $languages as $lang)
                                <div class="tab-pane fade {{$loop->first?'show active': ''}}" id="tsa_tab_pane_{{$lang->id}}">
                                    ...
                                    <x-backend.input.group :lang="$lang" label="متن صفحه" :error="$errors->first(getLocale($lang).'_text')">
                                       <textarea id="{{'innovation_content_'.getLocale($lang) }}"
                                                 class="form-control text-black @error( getLocale($lang).'_text') is-invalid @enderror"
                                                 rows="5"
                                                 name="{{getLocale($lang).'_text'}}">
                                           {{ old(getLocale($lang).'_text') ?  old(getLocale($lang).'_text') : $content->translate(getLocale($lang))->text}}
                                       </textarea>
                                    </x-backend.input.group>
            ...
            @push('scripts')
                ...
                <script>
                    ...
                    ClassicEditor.create(document.querySelector('#innovation_content_{{ getLocale($lang) }}'))
                        .then(editor => {})
                        .catch(error => {})
